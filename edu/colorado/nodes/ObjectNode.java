 ...
 *       the answer from listLength is incorrect because of arithmetic overflow.
 *
 * @see <A HREF="../../../../edu/colorado/nodes/ObjectNode.java"> Java Source
 *      Code for this class
...
public class ObjectNode {
	 ...
	 *                    reference may be null to indicate that there is no node
	 *                    after this new node.
	 ...
	 * @postcondition A new node has been created and placed after this node. The
	 *                data for the new node is item. Any other nodes that used to be
	 *                after this node are now after the new node.
	 * @exception OutOfMemoryError Indicates that there is insufficient memory for a
	 *                             new ObjectNode.
	 * @big_theta_notation 成(1)
	 * 
	 **/
	 ...
	 * Copy a list.
	 * 
	 * @param source the head of a linked list that will be copied (which may be an
	 *               empty list in where source is null)
	 * @precondition The source linked list should have been already initialized and
	 *               the list is null terminated
	 * @return The method has made a copy of the linked list starting at source. The
	 *         return value is the head reference for the copy.
	 * @exception OutOfMemoryError Indicates that there is insufficient memory for
	 *                             the new list.
	 * @big_theta_notation 成(n)
	 * 
	 **/
	 ...
	 *               the list is null terminated
	 * @return The method has made a copy of the linked list starting at source. The
	 *         return value is an array where the [0] element is a head reference
	 ...
	 *         specified start node to the specified end node. The return value is
	 *         an array where the [0] component is a head reference for the copy and
	 *         the [1] component is a tail reference for the copy.
	 ...
	 * @exception OutOfMemoryError         Indicates that there is insufficient
	 *                                     memory for the new list.
	 * @big_theta_notation 成(n)
	 * 
	 **/
	 ...
	 * @precondition position &ge; 0.
	 * @return The return value is a reference to the node at the specified position
	 *         in the list. (The head node is position 1, the next node is position
	 *         2, and so on.) If there is no such position (because the list is too
	 *         short), then the null reference is returned.
	 * @exception IllegalArgumentException Indicates that position is not positive.
	 * @big_theta_notation 成(n)
	 * 
	 **/
	 ...
	 * @exception NullPointerException Indicates that this was the tail node of the
	 *                                 list, so there is nothing after it to remove.
	 * @big_theta_notation 成(1)
	 **/
	...
	public static ObjectNode listCopy_rec(ObjectNode source) {
		...
		// Check if reaching the end of the source linked list.
		if (source.link != null) {
	...
	/**
	 * Display all the data on on the list
	 * 
	 * 
	 * @param head the head reference for a linked list (which may be an empty list
	 *             with a null head)
	 * @precondition The list is null terminated
	 * @return Java String containing all of the data on the list or null if the
	 *         list is empty
	 * @big_theta_notation 成(n)
	 * 
	 **/
	public String toString() {
		...
		for (cursor = link; cursor != null; cursor = cursor.link) {
			DataSubstring = cursor.data + "";
			DataString = DataString + DataSubstring;
		}
		return DataString;
	...
	public static void main(String[] args) {
		// a
		ObjectNode alphbet = new ObjectNode('A', null);
		ObjectNode cursor;
		char letter;
		for (cursor = alphbet, letter = 'B'; letter <= 'Z'; cursor = cursor.link, letter = (char) (letter + 1)) {
			cursor.addNodeAfter(letter);
		}

		// b
		System.out.println();
		System.out.println(alphbet.toString());

		// c
		System.out.println("Number of Nodes = " + listLength(alphbet));
		System.out.println("Number of Nodes = " + listLength_rec(alphbet));
		...
		System.out.println("Number of Nodes in r = " + listLength(k));
		System.out.println("Number of Nodes in r = " + listLength_rec(k));

		// g
		ObjectNode k2;
		k2 = listCopy_rec(alphbet);
		...
		System.out.println("Number of Nodes in r2 = " + listLength(k));
		System.out.println("Number of Nodes in r2 = " + listLength_rec(k));
